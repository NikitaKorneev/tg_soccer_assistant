...
class PollOpenCommandHandler(BaseCommandHandler):
    ...
    async def handle(self, message: Message, *args, **kwargs):
        ...
        call_to_arms = (
            f"‼️Мужчины‼️ Многоуважаемым @{message.from_user.username} созывается спортивная сессия‼️\n\n"
            # Метод strip('/start_poll') удаляет каждый символ из набора, а не всю подстроку.
            # Поэтому он может случайно удалить нужные символы с начала и конца строки.
            # Лучше так:
            f"📍Время, место и дисциплина⌚️:{message.text.replace('/start_poll', '', 1).strip()}📍"
        )
